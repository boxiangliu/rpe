...
make_cluster_plot <- function(
  ...
  # 1 chr12 56115278 56115473 clu_1202 56115376            annotated 1.029    b
  # 2 chr12 56115278 56117670 clu_1202 56116474 novel annotated pair 2.888    a
  # 3 chr12 56115731 56117670 clu_1202 56116700 novel annotated pair 0.963    c
  ...
  coords <- c(0,cumsum(trans_d)) # this coordinate will be used to transform genomic coordinate to plot coordinate.  
  names(coords)=s
  ...
  maxratio=0
  minratio=1.0
  yFactor = 0.65   # originally set as 0.65. This can be modified to change the height of curves
  yConstant = 0.5 # originally set as 0.5. This can be modified to change the height of curves
  ...
  summary_func2 = function(a) apply(a, 2, function(g) mean(g, na.rm=T))
  ...
  for( fancyVar in 1:length(groups) ){
    ...
    intron_meta$averageCounts=summary_func2(y[ groups[fancyVar]==x,,drop=F])
    ...
    allEdges=do.call(rbind,foreach (i=1:nrow(intron_meta)) %do% {
      ...
      edge$label=sprintf('%d (%.01f%%)',round(intron_meta$averageCounts[i]),intron_meta$prop[i]*100)
    ...
    new_theme_empty +
      ...
      # label the junctions
      geom_label(data=allEdgesP,aes(x=xtext,y=0.95*ytext,label=label), size = labelTextSize, label.size = NA, parse=FALSE, fill = "white",colour = "black", label.r = unit(0.3,"lines"), label.padding = unit(0.3,"lines") ) +
      geom_label(data=allEdges,aes(x=xtext,y=0.95*ytext,label=label), size= labelTextSize, label.size = NA, parse=FALSE, fill = "white", colour = "black", label.r = unit(0.3,"lines"), label.padding = unit(0.3,"lines") ) +
      #
      ylim(YLIMN,YLIMP) +
