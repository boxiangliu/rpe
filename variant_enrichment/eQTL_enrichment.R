...
in_dir='../processed_data/rasqual/output/glucose/joint/'
vcf_dir='/srv/persistent/bliu2/HCASMC_eQTL/processed_data/gwas_atacseq_overlap/prepare_vcf/'
anno_fn='../processed_data/variant_annotation/snpEff/all.bed.gz'
out_dir='../processed_data/variant_enrichment/eQTL_enrichment/'
fig_dir='../figures/variant_enrichment/eQTL_enrichment/'
...
select_top_eqtl=function(eGenes,in_dir){
	top_eqtl=foreach(i=eGenes,.combine='rbind')%dopar%{
		message('INFO - ',i)
		fn=list.files(in_dir,pattern=i,full.names=TRUE,recursive=TRUE)
...
main=function(){
	...
	eGenes = treeQTL[glucose==1,gene]
	top_eqtl = select_top_eqtl(eGenes,in_dir)
