in_dir='../data/genotype/asvcf/glucose/'
out_dir='../data/genotype/asvcf/glucose_sid/'
...
in_dir='../data/genotype/asvcf/galactose/'
out_dir='../data/genotype/asvcf/galactose_sid/'
if [[ ! -d $out_dir ]]; then mkdir -p $out_dir; fi
for f in $(ls $in_dir/*vcf.new.gz); do
	vcf_fn=$(basename $f)
	echo $vcf_fn
