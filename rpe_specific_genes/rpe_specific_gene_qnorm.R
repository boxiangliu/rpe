...
main=function(){
	...
	rpkm_mat=as.matrix(rpkm[,2:ncol(rpkm)])

	qnorm_rpkm=normalize.quantiles(rpkm_mat,copy=TRUE)
	qnorm_rpkm=as.data.table(qnorm_rpkm)
	qnorm_rpkm=cbind(rpkm$Name,qnorm_rpkm)
	colnames(qnorm_rpkm)=colnames(rpkm)
	rpkm=qnorm_rpkm
	...
	fwrite(out,paste0(out_dir,'/all_genes_qnorm.txt'),sep='\t')
	fwrite(out[zscore>4,],paste0(out_dir,'/rpe_specific_genes_qnorm.txt'),sep='\t')
}
